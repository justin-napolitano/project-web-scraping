...
def initial(df,sheets_service):
    start = 2
    num_rows = 2
    num_pages = 1
    for url in df['url'].to_list():
        start,num_rows = scrape(url,sheets_service,start,num_rows,num_pages)

def scrape(url,sheets_service,start,num_rows,num_pages):
    page_num = str(num_pages)


    
    search_url = "/".join([url, page_num])
    pprint(search_url)
    spreadsheet_id = '<for me to know>'
    #num_rows = 2
    #start = 2
    num_columns = 0
    seen_columns = {}
   ...
   #attrs={'data-bin' : True}
    ...
    if paginator_next is not None:
        ...
        if href is not None:
            ...
            #nxt = href['href']
            #print(type(url))
            scrape(url,sheets_service,start,num_rows,num_pages)
        else:
            pass
    else:
        print("Done Scraping")
...
def about_test(agent_column):
    ...
    for k,v in agent_column.to_dict().items():
        time.sleep(random.randint(5,15)) 
        print(k,v)
        search_url = "<your url here>"+ v + about
